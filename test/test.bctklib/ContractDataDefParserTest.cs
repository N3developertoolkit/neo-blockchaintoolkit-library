...
namespace test.bctklib
{
    public class ContractDataDefParserTest
    {
        ...
        public void test_ParseStructs()
        {
            ...
            var us = ContractDataDef.ParseStructs(doc.RootElement);
            var bs = ContractDataDef.BindStructs(us);

            var expect1 = new StructDef
            {
                Name = "TokenState",
                Fields = new[] {
            ...
            var actual1 = bs.Single(s => s.Name == expect1.Name);
            var actual2 = bs.Single(s => s.Name == expect2.Name);

            bs.Should().HaveCount(2);
            actual1.Should().BeEquivalentTo(expect1);
            actual2.Should().BeEquivalentTo(expect2);
        }
        ...
        public void test_invalid_type_throws()
        {
            ...
            var us = ContractDataDef.ParseStructs(doc.RootElement);
            Assert.Throws<Exception>(() => ContractDataDef.BindStructs(us));
        }
